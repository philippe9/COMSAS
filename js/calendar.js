...
calendarTable += '<td ' + id + '>' +'<a data-toggle="tooltip"'+'title="Super event du comsas">'+ i +'</a>'+ '</td>'; border++;
 } 
else { id = '';calendarTable += '<td ' + id + '>' + i + '</td>'; border++; }
...
for ( var x = 0; x <= today.getMonthNames().length-1; x++ ) { selected_month += '<option value="' + today.getMonthNames()[x] + ' 1, ' + today.getFullYear() + '">' + today.getMonthNames()[x] + '</option>'; }
